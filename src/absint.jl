...
function actual_size(@nospecialize(typ2))::Int
    ...
    if typ2 <: String || typ2 <: Symbol || typ2 <: Core.SimpleVector || typ2 <: Core.Module
        return sizeof(Int)
...
"""
    gep_candidate_offsets(arg::LLVM.GetElementPtrInst, maxcandidates::Int = 16)

For a getelementptr whose indices are not all constant, enumerate the byte offsets it may
take relative to its pointer operand. A non-constant index is bounded by the static extent
of the array/vector it indexes into. Julia only emits such a gep for an in-range element
access (e.g. `emit_getfield_unknownidx` for `getfield` with a non-constant symbol), so
this bound holds for julia-generated code.

Returns `nothing` if no bound can be established (in particular if the leading index,
which strides over the source element type itself, is non-constant), if the gep is not
inbounds, if every index is constant, or if there would be more than `maxcandidates`
offsets.
"""
function gep_candidate_offsets(arg::LLVM.GetElementPtrInst, maxcandidates::Int = 16)::Union{Nothing, Vector{Int}}
    if LLVM.API.LLVMIsInBounds(arg) == 0
        return nothing
    end
    ...
    # the leading index strides over the source element type itself, and as such is not
    # bounded by any static extent
    if !isa(indices[1], LLVM.ConstantInt)
        return nothing
    end
    ...
    # the constant part of the offset, and the (stride, extent) of each variable index
    base = convert(Int, indices[1]) * Int(LLVM.abi_size(dl, source_type))
    strides = Int[]
    extents = Int[]
    ...
    for i in 2:length(indices)
        idx = indices[i]
        if isa(cur, LLVM.ArrayType) || isa(cur, LLVM.VectorType)
            elem = eltype(cur)
            stride = Int(LLVM.abi_size(dl, elem))
            if isa(idx, LLVM.ConstantInt)
                base += convert(Int, idx) * stride
            else
                n = length(cur)
                if n <= 0
                    return nothing
            ...
            cur = elem
        elseif isa(cur, LLVM.StructType)
            # llvm requires the index into a struct to be constant
            if !isa(idx, LLVM.ConstantInt)
                return nothing
            end
            eidx = convert(Int, idx)
            if eidx < 0 || eidx >= length(elements(cur))
                return nothing
            end
            base += Int(LLVM.offsetof(dl, cur, eidx))
            cur = elements(cur)[eidx + 1]
        else
            return nothing
        end
    end

    # every index was constant, the caller's existing constant offset logic applies
    if ncombos == 1
        return nothing
    end

    offsets = Int[]
    for k in 0:(ncombos - 1)
        rest = k
        off = base
        for i in 1:length(extents)
            off += (rest % extents[i]) * strides[i]
            rest = rest ÷ extents[i]
        end
        push!(offsets, off)
    end
...
"""
    abs_typeof_load_at(legal, typ, byref, offset, argty, dl)

Given the julia type `typ` (with calling convention `byref`) of the base object a load
reads from, return the julia type of a load of llvm type `argty` at byte `offset` into it.
"""
function abs_typeof_load_at(
        legal::Bool,
        @nospecialize(typ),
        @nospecialize(byref),
        offset::Int,
        @nospecialize(argty::LLVM.LLVMType),
        dl::LLVM.DataLayout
    ...
    if typ <: Ptr && Base.isconcretetype(typ) && byref == GPUCompiler.BITS_VALUE
        ET = eltype(typ)
    ...
    if (byref == GPUCompiler.MUT_REF || byref == GPUCompiler.BITS_REF) && Base.isconcretetype(typ)
        if shouldLoad
            byref = GPUCompiler.BITS_VALUE
        end

        legal = true
        sz = argty == LLVM.IntType(1) ? 1 : sizeof(dl, argty)
        is_padded = false
        ...
        typ2 = typ
        while legal && should_recurse(typ2, argty, byref, dl)
            if is_padded
                break
            end
            if !Base.isconcretetype(typ2)
...
function abs_typeof(
    ...
    if isa(arg, LLVM.LoadInst)
        ...
        # A gep with a non-constant index (as julia emits for getfield with a non-constant
        # symbol/index) does not have a single byte offset. Enumerate the offsets it may
        # take and only conclude a type if every one of them loads the same type.
        offsets = Int[offset]
        if isa(larg, LLVM.GetElementPtrInst) &&
                !all(Base.Fix2(isa, LLVM.ConstantInt), operands(larg)[2:end])
            candidates = gep_candidate_offsets(larg)
            if candidates !== nothing
                base, base_offset = get_base_and_offset(operands(larg)[1])
                empty!(offsets)
                for c in candidates
                    push!(offsets, offset + base_offset + c)
                end
                larg = base
            end
        end
        ...
        for cur_offset in offsets
            cur_legal, cur_typ, cur_byref =
                abs_typeof_load_at(legal, typ, byref, cur_offset, value_type(arg), dl)
            if !cur_legal
                found_legal = false
                break
            end
            if found_typ === nothing
                found_typ = cur_typ
                found_byref = cur_byref
                found_legal = true
            elseif found_typ != cur_typ || found_byref != cur_byref
                found_legal = false
                break
            end
        end
        if found_legal
            return (true, found_typ::Type, found_byref::GPUCompiler.ArgumentCC)
        end
    end
