...
function emit_apply_type!(B::LLVM.IRBuilder, @nospecialize(Ty::Type), args::Vector{LLVM.Value}, world::UInt, enzyme_context::Union{EnzymeContext, Nothing} = nothing)::LLVM.Value
    curent_bb = position(B)
    fn = LLVM.parent(curent_bb)
    mod = LLVM.parent(fn)
...
function emit_tuple!(B::LLVM.IRBuilder, args::Vector{LLVM.Value}, world::UInt, enzyme_context::Union{EnzymeContext, Nothing} = nothing)::LLVM.Value
    curent_bb = position(B)
    fn = LLVM.parent(curent_bb)
    mod = LLVM.parent(fn)
