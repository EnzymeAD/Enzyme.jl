...
@static if Interpreter.HAS_INVOKE_RULES
    ...
    function check_rule_specsig(llvmf::LLVM.Function, mi::Core.MethodInstance, @nospecialize(RT::Type))
        ...
        if !ok
            msg = sprint() do io
                println(io, "Enzyme: the emitted rule does not match the derived signature")
                println(io, "mi = ", mi)
    ...
    inlineable. A rule with a constant result gets `:inline` whatever its
    ...
    [`native_rules_available`](@ref)), and for rules with the `:inline`
    convention.

    Throw a `CallingConventionMismatchError` when Julia compiled the rule for
