...
function body_runtime_generic_fwd(N, Width, wrapped, primtypes)
    ...
    return quote
        ...
        world = GPUCompiler.tls_world_age() #FT, tt)
...
function body_runtime_generic_augfwd(N, Width, wrapped, primttypes)
    ...
    return quote
        ...
        world = GPUCompiler.tls_world_age() #FT, Tuple{$(ElTypes...)})
...
function GPUCompiler.codegen(output::Symbol, job::CompilerJob{<:EnzymeTarget};
    ...
    mod, meta = JuliaContext() do ctx
        GPUCompiler.codegen(:llvm, primal_job; optimize=false, cleanup=false, validate=false, parent_job=parent_job)
    end
