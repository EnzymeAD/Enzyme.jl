...
function generic_setup(orig, func, ReturnType, gutils, start, B::LLVM.IRBuilder,  lookup; sret=nothing, tape=nothing, firstconst=false, endcast=true, firstconst_after_tape=true)
    ...
    for (i, op) in enumerate(ops)
        ...
        if !active
            push!(ActivityList, unsafe_to_llvm(mod, false))
        else
            ...
            if API.runtimeActivity()
                ...
                push!(ActivityList, select!(B, icmp!(B, LLVM.API.LLVMIntNE, val, inv_0), unsafe_to_llvm(mod, true), unsafe_to_llvm(mod, false)))
            else
                push!(ActivityList, unsafe_to_llvm(mod, true))
            end
        end
    ...
    if tape !== nothing
        ...
        pushfirst!(vals, unsafe_to_llvm(mod, Val(ReturnType)))
    end
    ...
    if mode != API.DEM_ForwardMode
        ...
        pushfirst!(vals, unsafe_to_llvm(mod, Val((ModifiedBetween...,))))
    end

    pushfirst!(vals, unsafe_to_llvm(mod, Val(Int(width))))
    ...
    pushfirst!(vals, unsafe_to_llvm(mod, func))
