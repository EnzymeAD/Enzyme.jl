            ...
            if B !== nothing
                ...
                if mixed
                    ...
                    for idx = 1:width
                        ...
                        metadata(ld)["enzyme_mustcache"] = MDNode(LLVM.Metadata[])
                        if reverse && uncacheable[arg.codegen.i] != 0
                            # The shadow's by-ref memory was overwritten after this call,
                            # so in the reverse pass only its float fields (the live
                            # derivative accumulators) are meaningful. Take the other
                            # inline data, which the shadow must mirror from the primal,
                            # from the cached primal value instead.
                            @assert val !== nothing
                            @assert value_type(val) == llrty
                            tmp = alloca!(alloctx, llrty, "rules_mixed_shadow_tmp")
                            store!(B, val, tmp)
                            copy_floats_into!(B, llrty, tmp, ev)
                            ld = load!(B, llrty, tmp, "rules_mixed_shadow_merged")
                        end
                        if n_primal_roots > 0
