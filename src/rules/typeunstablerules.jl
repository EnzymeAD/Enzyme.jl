...
function common_jl_getfield_augfwd(offset, B, orig, gutils, normalR, shadowR, tapeR)::Bool
    ...
    vals = LLVM.Value[unsafe_to_llvm(mod, AA)]
    push!(vals, inps[1])
    ...
    push!(vals, unsafe_to_llvm(mod, Val(is_constant_value(gutils, ops[2]))))

    for v in inps[2:end]
        push!(vals, v)
    end

    pushfirst!(vals, unsafe_to_llvm(mod, rt_jl_getfield_aug))
...
function common_jl_getfield_rev(offset, B, orig, gutils, tape)
    ...
    pushfirst!(vals, unsafe_to_llvm(mod, rt_jl_getfield_rev))
...
@register_aug function jl_nthfield_augfwd(B, orig, gutils, normalR, shadowR, tapeR)
    ...
    push!(vals, unsafe_to_llvm(mod, Val(is_constant_value(gutils, ops[1]))))

    for v in inps[2:end]
        push!(vals, v)
    end

    pushfirst!(vals, unsafe_to_llvm(mod, idx_jl_getfield_aug))
...
@register_rev function jl_nthfield_rev(B, orig, gutils, tape)
    ...
    pushfirst!(vals, unsafe_to_llvm(mod, idx_jl_getfield_rev))
    ...
    if !is_constant_value(gutils, origops[2])
        ...
        for idx in 1:width
            vals = LLVM.Value[
              ...
              new_from_original(gutils, origops[3]),
              unsafe_to_llvm(mod, Val(is_constant_value(gutils, origops[4]))),
              new_from_original(gutils, origops[4]),
              is_constant_value(gutils, origops[4]) ? unsafe_to_llvm(mod, nothing) : ((width == 1) ? shadowval : extract_value!(B, shadowval, idx-1)),
            ]
            
            pushfirst!(vals, unsafe_to_llvm(mod, rt_jl_setfield_aug))
    ...
    if !is_constant_value(gutils, origops[2])
        ...
        for idx in 1:width
            vals = LLVM.Value[
              ...
              is_constant_value(gutils, origops[4]) ? unsafe_to_llvm(mod, nothing) : lookup_value(gutils, ((width == 1) ? shadowval : extract_value!(B, shadowval, idx-1)), B),
            ]
            
            pushfirst!(vals, unsafe_to_llvm(mod, rt_jl_setfield_rev))
...
function common_f_svec_ref_augfwd(offset, B, orig, gutils, normalR, shadowR, tapeR)
    ...
    shadowres = if width == 1
        ...
        emit_apply_generic!(B, LLVM.Value[unsafe_to_llvm(mod, errfn), emit_jltypeof!(B, cal)])
        cal
    else
        ST = LLVM.LLVMType(API.EnzymeGetShadowType(width, value_type(orig)))
        shadow = LLVM.UndefValue(ST)
