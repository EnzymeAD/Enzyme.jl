...
function call_samefunc_with_inverted_bundles!(B::LLVM.IRBuilder, gutils::GradientUtils, orig::LLVM.CallInst, args::Vector{<:LLVM.Value}, valTys::Vector{API.CValueType}, lookup::Bool)
    ...
    return LLVM.Value(API.EnzymeGradientUtilsCallWithInvertedBundles(gutils, LLVM.called_operand(orig), LLVM.called_type(orig), args, length(args), orig, valTys, length(valTys), B, #=lookup=#false))
end
...
get_runtime_activity(gutils::GradientUtils) = API.EnzymeGradientUtilsGetRuntimeActivity(gutils)
