...
num2num_4(x) = 1.0
num2num_5(x) = 1.0 / (1.0 + exp(-x))

const NUMBER_TO_NUMBER_FUNCS = (
    num2num_1,
    num2num_3, #=num2num_2,=#
    num2num_4,
    num2num_5,
...
function ackley(x)
    ...
    return (-a * exp(b * sqrt(len_recip * sum_sqrs)) - exp(len_recip * sum_cos) + a + exp(1))
end

self_weighted_logit(x) = inv(1.0 + exp(-dot(x, x)))

const VECTOR_TO_NUMBER_FUNCS = (
    vec2num_1,
    vec2num_2,
    vec2num_3,
    vec2num_4,
...
const INPLACE_ARRAY_TO_ARRAY_FUNCS =
    (chebyquad!, brown_almost_linear!, trigonometric!, mutation_test_1!, mutation_test_2!)
...
const ARRAY_TO_ARRAY_FUNCS =
    (-, chebyquad, brown_almost_linear, trigonometric, arr2arr_1, arr2arr_2, mutation_test_1, mutation_test_2, identity)
