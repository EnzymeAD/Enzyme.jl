...
const POP_CONST_DEFAULT = [0.0]

@testset "pop! with a constant default and runtime activity" begin
    # The default is constant and the stored value is active.
    # With runtime activity, the rule uses the primal default as its shadow.
    function f_pop_const_default(x::Vector{Float64})
        d = IdDict{Symbol, Vector{Float64}}()
        d[:a] = x
        return pop!(d, :a, POP_CONST_DEFAULT)[1] * 3.0
    end

    x = [1.5]
    dx = [0.0]
    autodiff(set_runtime_activity(Reverse), f_pop_const_default, Active, Duplicated(x, dx))
    @test dx[1] ≈ 3.0

    fwd = autodiff(set_runtime_activity(Forward), f_pop_const_default, Duplicated, Duplicated([1.5], [1.0]))[1]
    @test fwd ≈ 3.0
end

@testset "pop! from a shadow table that does not have the key" begin
    # The primal table has the key, but the shadow table does not.
    # The rule must not return the shadow default as the shadow of the popped value.
    f_pop_shadow_missing(d, x) = pop!(d, :a, x)[1] * 3.0

    d = IdDict{Symbol, Vector{Float64}}(:a => [2.0])
    dd = IdDict{Symbol, Vector{Float64}}()
    x = [1.5]
    dx = [0.0]
    @test_throws Enzyme.Compiler.EnzymeRuntimeException autodiff(
        Reverse, f_pop_shadow_missing, Active, Duplicated(d, dd), Duplicated(x, dx)
    )
end
