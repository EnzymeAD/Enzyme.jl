...
@testset "Sort rules" begin
    ...
    @test autodiff(Forward, f3, BatchDuplicated(1.5, (1.0, 2.0)))[1] ==
          (var"1"=1.0, var"2"=2.0)
    ...
    @static if VERSION < v"1.7-" || VERSION >= v"1.8-"
        @test autodiff(Forward, f4, BatchDuplicated(1.5, (1.0, 2.0)))[1] ==
              (var"1"=1.5, var"2"=3.0)
    end
    ...
    dd = Duplicated([TPair(1, 2), TPair(2, 3), TPair(0, 1)],
                    [TPair(0, 0), TPair(0, 0), TPair(0, 0)])
...
@static if VERSION > v"1.8"
    ...
    @testset "Linear solve for triangular matrices" begin
        @testset for T in (UpperTriangular, LowerTriangular, UnitUpperTriangular,
                           UnitLowerTriangular),
            ...
            @testset "test through `Adjoint` wrapper (regression test for #1306)" begin
                ...
                dY1 = rand(TE, sizeB...)
                dY2 = copy(dY1)
                autodiff(Reverse, f!, Duplicated(Y, dY1), Duplicated(A1, dA1),
                         Duplicated(B, dB1))
                autodiff(Reverse, f!, Duplicated(Y, dY2), Duplicated(A2, dA2),
                         Duplicated(B, dB2))
...
@testset "rand and randn rules" begin
    ...
    @test autodiff(Enzyme.Reverse, x -> rand(MyDistribution(x)), Active, Active(1.0)) ==
          ((1.0,),)
end
