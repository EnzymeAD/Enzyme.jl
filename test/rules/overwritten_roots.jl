...
function EnzymeRules.augmented_primal(config::EnzymeRules.RevConfigWidth{1},
        ::Const{typeof(red_int)}, ::Type{RT}, A::Annotation) where {RT}
    ret = red_int(A.val)
    return EnzymeRules.AugmentedReturn(EnzymeRules.needs_primal(config) ? ret : nothing,
...
function EnzymeRules.augmented_primal(config::EnzymeRules.RevConfigWidth{2},
        ::Const{typeof(red_int)}, ::Type{RT}, A::Annotation) where {RT}
    ret = red_int(A.val)
    return EnzymeRules.AugmentedReturn(EnzymeRules.needs_primal(config) ? ret : nothing,
        EnzymeRules.needs_shadow(config) ? (zero(ret), zero(ret)) : nothing, nothing)
end
function EnzymeRules.reverse(::EnzymeRules.RevConfigWidth{2},
...
loop_int(a) = (s = 0.0; for i in 1:4; s += red_int(mk_int(a, i)); end; s)
...
function EnzymeRules.augmented_primal(config::EnzymeRules.RevConfigWidth{1},
        ::Const{typeof(red_mixed)}, ::Type{RT}, A::Annotation) where {RT}
    ret = red_mixed(A.val)
    return EnzymeRules.AugmentedReturn(EnzymeRules.needs_primal(config) ? ret : nothing,
        EnzymeRules.needs_shadow(config) ? zero(ret) : nothing, nothing)
end
...
loop_mixed(a) = (s = 0.0; for i in 1:4; s += red_mixed(mk_mixed(a, i)); end; s)
